//! Memory Test: no_std crates (yrs_no_std + yrs-warp-nostd) with TLS
//!
//! Identical to memory_test_nostd but uses TLS (tokio-rustls) instead of
//! plain TCP for client connections.
//!
//! Run with: cargo run --example memory_test_nostd_tls
// CERT_PATH=certs/cert.pem KEY_PATH=certs/key.pem cargo run --example memory_test_nostd_tls
...
unsafe impl GlobalAlloc for TrackingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        // Check if we would exceed the 1 GB limit
        let current = HEAP_ALLOC
            .load(Ordering::Relaxed)
            .saturating_sub(HEAP_DEALLOC.load(Ordering::Relaxed));
...
/// TLS stream type alias for brevity
type TlsStream = tokio_rustls::server::TlsStream<TcpStream>;

async fn handle_connection(stream: TlsStream, rooms: Rooms) {
    ...
    // Split the TLS stream into independent read/write halves using tokio::io::split
    ...
    println!("[{}] Client connected to room: {} (TLS)", peer_addr, room);
    ...
    // Spawn FRAMING tasks
    struct WsReader {
        read: tokio::io::ReadHalf<TlsStream>,
        ws: WebSocketConn,
        buf: Vec<u8>,
    }
    impl WsReader {
    ...
    struct WsWriter {
        write: tokio::io::WriteHalf<TlsStream>,
        ws: WebSocketConn,
    }
    impl WsWriter {
    ...
    println!("[{}] Client disconnected from room: {} (TLS)", peer_addr, room);
    log_heap_stats(&format!("After disconnect {}", peer_addr));
}

// ============================================================================
// TLS Config (self-signed — client skips verification)
// ============================================================================
...
fn main() {
    ...
    rt.block_on(async {
        println!("=================================================");
        println!("  Memory Test: no_std crates (TLS)");
        println!("  (yrs_no_std + yrs-warp-nostd + tokio-rustls)");
        println!("=================================================");
        println!();
        ...
        // Generate a throwaway self-signed cert — client skips verification
        ...
        println!("Server listening on wss://0.0.0.0:5560/collaboration/<room>");
        println!("Using self-signed cert (client verification disabled).");
                ...
                Ok((tcp_stream, _)) => {
                    ...
                    tokio::spawn(async move {
                        match acceptor.accept(tcp_stream).await {
                            Ok(tls_stream) => {
                                handle_connection(tls_stream, rooms).await;
                            }
                            Err(e) => {
